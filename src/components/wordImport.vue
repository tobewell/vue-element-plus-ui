<!--Word文件批量导入规范通用组件-->
<template> 
	<el-form class="" v-loading="importLoading" element-loading-text="正在导入word文件">
		<el-form-item  :label="'导入' + targetName + 'Word模板：'">
		...
		<el-form-item  :label="'导入'+ targetName +'Word模板：'">
			<upload ref="upload_Customer" style="max-width: 500px;"
					...
					<div slot="tip" class="el-upload__tip"> 
						<slot>
							<div><i class="el-icon-info mr8"></i>只能上传docx文件，且不超过3m</div>
						</slot>
						<slot name="otherInfo">
						</slot>
					</div>
...
export default {
	...
	props: {
		...
		filters:{
			type: Object,
			default:function (){
				return 	 {ext: 'doc,docx',max_file_size: '3mb'};
			}
		},
	},
	data() {
